...
def main():
    ...
    palindromes = [ "Go hang a salami Im a lasagna hog", "Able was I ere I saw Elba",
...
def list_count( item, a_list ):
    ...
    if not a_list:
        ...
        if first( a_list ) == item:
            count += 1
            return count + list_count( item, rest( a_list ) )
        else:
...
def list_merge( a_list, b_list ):
    ...
    if first( a_list ) < first( b_list ):
        result.append( first( a_list ))
        return result + list_merge( rest( a_list ), b_list)

    elif first( a_list ) > first( b_list ):
        result.append( first( b_list ))
        return result + list_merge( a_list, rest( b_list ))
