...
def draw_sierpinski_triangle( artist ):
    ...
    sierpinski_triangle( artist, triangle, 7 )
...
def draw_sierpinski_carpet( artist ):
    ...
    sierpinski_carpet( artist, 0, 0, WIDTH, HEIGHT, 4 )
...
def draw_sierpinski_circles( artist ):
    ...
    sierpinski_circles( artist, 0, 0, ( WIDTH + HEIGHT ) // 8, 5 )
...
def draw_h_tree( artist ):
    ...
    h_tree( artist, 0, 0, ( WIDTH + HEIGHT ) // 4, 5 )
...
def leafy_tree( artist, branch_length, depth ):
    ...
    if depth <= 0 or branch_length < 4:
        ...
        artist.pencolor(random.choice(FALL_COLORS))
        artist.dot(6)
...
def snowflake_side( artist, length, depth ):
    ...
    if depth <= 1 or length < 10:
        artist.forward( length )
    else:
        artist.pensize( depth )
        artist.forward( length // 3)
        artist.left( 60 )
        koch_snowflake( artist, length * 1 // 3, depth - 1 )
        artist.right(60)
        artist.forward( length * 2 // 3)
